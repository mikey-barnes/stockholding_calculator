...
def non_movers_report():
    with open("sales_data.csv", errors="ignore", newline="") as sales_csv, open(
        ...
        non_movers_sku_list = [
            sku for sku in products_sku_list if sku not in sales_sku_list
        ]

    with open("product-export.csv", errors="ignore", newline="") as products_csv, open(
        "non_movers.csv", "w", newline=""
    ) as non_movers_csv:
        products_reader = csv.DictReader(products_csv)
